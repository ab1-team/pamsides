            ...
            <table class="report-table">
              ...
              <tbody>
                <template v-for="row in rows" :key="row.key">
                  ...
                  <tr v-else-if="row.type === 'lev3'" class="detail-row" :class="{ 'zebra-bg': row.isEven }">
                    <td>{{ row.kode_akun }}.</td>
                    <td>{{ row.nama_akun }}</td>
                    <td class="text-right">{{ formatCurrency(row.saldo) }}</td>
                  </tr>

                  <tr v-else-if="row.type === 'lev4'" class="detail-row" :class="{ 'zebra-bg': row.isEven }">
...
.detail-row { background-color: #e4e4e4; page-break-inside: avoid; break-inside: avoid; }
