    ...
    <BaseReportLayout :config="payload?.config">
        <div class="header-section" style="text-align:center;margin-bottom:15px;font-family:sans-serif;">
            <h2 style="margin:0;font-size:14pt;font-weight:bold;text-transform:uppercase;color:#000;">
                {{ payload?.periode?.bulan === '0' ? 'NERACA AWAL TAHUN' : 'NERACA' }}
            </h2>
        ...
        <table class="report-table">
            ...
            <tbody>
                <template v-for="row in rows" :key="row.key">
                    ...
                    <tr v-if="row.type === 'total'" class="spacer-row">
                        <td colspan="3">&nbsp;</td>
                    </tr>
    ...
    const rows = computed(() => {
        ...
        items.forEach((lev1, i1) => {
            ...
            out.push({
                type: 'total',
                nama_akun: lev1.nama_akun,
                total_saldo: lev1.total_saldo_lev1 || 0,
                key: k1 + '-t'
            })
        })
        return out
...
<style scoped>
    ...
    /* PERUBAHAN DI SINI: Warna lebih kontras agar efek zebra terlihat */
    .detail-row {
