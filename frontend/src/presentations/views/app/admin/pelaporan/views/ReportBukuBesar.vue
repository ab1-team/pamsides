<template>
  <BaseReportLayout :config="payload?.config">
    <div class="header-section" style="text-align: center; margin-bottom: 15px; font-family: sans-serif; color: #000;">
    ...
    <table class="data-table report-table" style="margin-top: 0px;">
      ...
      <tbody>
              ...
              <tr v-if="showFooter" class="row-total">
                <td colspan="4">Total Transaksi Bulan {{ payload.periode.bulan_name }} {{ payload.periode.tahun }}</td>
              ...
              <tr v-if="showFooter" class="row-total">
                <td colspan="4">Total Transaksi sampai dengan Bulan {{ payload.periode.bulan_name }} {{ payload.periode.tahun }}</td>
...
const saldoAwalTahun = computed(() => {
  const data = props.payload?.saldo_awal_tahun || { debit: 0, kredit: 0 }
...
const saldoBulanLalu = computed(() => {
  const data = props.payload?.saldo_bulan_lalu || { debit: 0, kredit: 0 }
  const debit = Number(data.debit || 0)
  const kredit = Number(data.kredit || 0)
  const saldo = isKredit.value ? (kredit - debit) : (debit - kredit)
  return { debit, kredit, saldo }
...
const totalDebitKumulatif = computed(() => {
  return Number(saldoAwalTahun.value.debit || 0) + Number(saldoBulanLalu.value.debit || 0) + totalDebitBulanIni.value;
});

const totalKreditKumulatif = computed(() => {
  return Number(saldoAwalTahun.value.kredit || 0) + Number(saldoBulanLalu.value.kredit || 0) + totalKreditBulanIni.value;
});
const finalSaldo = computed(() => tableData.value.length
  ? tableData.value[tableData.value.length - 1].running_saldo
  : initialSaldo.value)

const periodeText = computed(() => `${(props.payload?.periode?.bulan_name || '').toUpperCase()} ${props.payload?.periode?.tahun || ''}`)
</script>
...
.report-table td {
  ...
  line-height: 1.2;
  color: #000;
  font-size: 8.5pt !important; /* Tambahkan ini agar memaksa ukuran */
}
.report-table th,
.report-table td {
  padding: 4px 6px;
...
.row-grey { background-color: #d1d1d1 !important; color: #000 !important; }
.row-total { background-color: #e6e6e6 !important; color: #000 !important; font-weight: bold; }
