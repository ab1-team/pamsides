...
const handlePrint = () => {
  ...
  if (!printWindow) {
    alert('Browser memblokir popup. Mohon izinkan popup untuk mencetak invoice.')
    return
  }

  const customer = getCustomerName.value
  const month = getMonthName(props.bill.billing_period_month)
  const year = props.bill.billing_period_year
  const total = new Intl.NumberFormat('id-ID').format(props.bill.total_amount)
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:Arial,sans-serif;padding:30px;color:#1e293b;font-size:13px;line-height:1.5}
  ...
  .row.total{border-top:2px solid #1e293b;margin-top:15px;padding-top:15px;font-weight:bold;font-size:15px}
  .label{color:#64748b}
