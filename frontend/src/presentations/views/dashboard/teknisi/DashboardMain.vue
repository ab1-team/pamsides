<template>
  <div class="teknisi-dashboard p-4! lg:p-8!">
    ...
    <div v-if="isLoading" class="flex! flex-col! items-center! justify-center! h-[60vh]! opacity-50!">
    ...
    <div v-else>
      ...
      <div class="grid! grid-cols-1! md:grid-cols-2! lg:grid-cols-4! gap-6! mb-10!">
        ...
        >
          ...
          <div class="w-12! h-12! bg-white/20! backdrop-blur-sm! rounded-xl! flex! items-center! justify-center! text-white! text-xl! mb-4! group-hover:scale-110! transition-transform!">
          ...
          <div class="mt-4! flex! items-center! gap-2! text-white! text-[10px]! font-bold! uppercase! tracking-widest!">
            Mulai Tugas <font-awesome-icon icon="arrow-right" class="group-hover:translate-x-1! transition-transform!" />
          </div>
        </div>
        ...
        >
          <div class="w-12! h-12! bg-amber-50! rounded-xl! flex! items-center! justify-center! text-amber-500! text-xl! mb-4! group-hover:scale-110! transition-transform!">
        ...
        <div class="bg-white! border! border-slate-100! p-6! rounded-3xl! shadow-sm! lg:col-span-2! flex! flex-col! justify-center!">
          <div class="flex! items-center! justify-between! mb-4!">
            ...
            <div class="text-right!">
              <div class="text-2xl! font-black! text-slate-800!">{{ readCustomers }} <span class="text-sm! font-bold! text-slate-400!">/ {{ totalCustomers }}</span></div>
            </div>
          </div>
          
          <div class="relative! w-full! h-3! bg-slate-100! rounded-full! overflow-hidden!">
            <div class="absolute! top-0! left-0! h-full! bg-gradient-to-r! from-emerald-400! to-emerald-500! rounded-full! transition-all! duration-1000!" :style="{ width: meterProgressPercentage + '%' }"></div>
          </div>
      ...
      <div class="grid! grid-cols-1! lg:grid-cols-3! gap-8!">
        ...
        <div class="lg:col-span-2! space-y-6!">
          <div class="flex! items-center! justify-between!">
            <h2 class="text-lg! font-black! text-slate-800! flex! items-center! gap-2!">
              <font-awesome-icon icon="clipboard-list" class="text-cyan-500!" /> Tugas Pemasangan / Survei
            </h2>
            <button 
          ...
          >
            <!-- Empty State -->
            <div v-if="displayedTasks.length === 0" class="text-center! py-10! bg-slate-50! rounded-2xl! border! border-dashed! border-slate-200!">
              <p class="text-sm! font-bold! text-slate-400!">Tidak ada tugas prioritas saat ini.</p>
            </div>

            <!-- Task Item -->
            <div v-for="(task, idx) in displayedTasks" :key="idx" class="bg-white! p-5! rounded-2xl! shadow-sm! border! border-slate-100! flex! flex-col! sm:flex-row! sm:items-center! justify-between! gap-4! transition-all! hover:shadow-md! hover:border-cyan-100!">
              <div class="flex! items-start! gap-4!">
                <div :class="`w-12! h-12! rounded-xl! shrink-0! flex! items-center! justify-center! text-lg! ${task.bgClass} ${task.textClass}`">
                  <font-awesome-icon :icon="task.icon" />
                </div>
                <div>
                  <div class="flex! items-center! gap-2! mb-1!">
                    <span :class="`text-[9px]! font-black! uppercase! tracking-widest! px-2! py-0.5! rounded-md! ${task.bgClass} ${task.textClass}`">{{ task.type }}</span>
                    <span class="text-xs! font-bold! text-slate-400!"><font-awesome-icon icon="clock" class="mr-1!" />{{ task.time }}</span>
                  </div>
                  <h4 class="text-sm! font-bold! text-slate-800! leading-snug!">{{ task.title }}</h4>
                  <p class="text-xs! text-slate-500! mt-1! line-clamp-1!"><font-awesome-icon icon="map-marker-alt" class="text-slate-300! mr-1!" /> {{ task.location }}</p>
                </div>
              </div>
              <div class="sm:text-right! shrink-0!">
                <BaseButton variant="secondary" size="sm" class="w-full! sm:w-auto! text-[10px]! font-black! uppercase!">Tindak Lanjut</BaseButton>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6!">
          ...
          <ContentCard variant="elevated" padding="none" class="border-0! shadow-xl! shadow-slate-200/40!">
            <div class="p-6! border-b! border-slate-50!">
              <h3 class="text-sm! font-bold! text-slate-800!">Statistik Sistem</h3>
            </div>
            <div class="p-6!">
              <div class="grid! grid-cols-2! gap-4!">
                <div v-for="(stat, idx) in techStats" :key="idx" class="bg-slate-50! p-4! rounded-2xl! text-center!">
                  <div :class="`w-8! h-8! rounded-lg! mx-auto! mb-2! flex! items-center! justify-center! ${stat.bg} ${stat.color}`">
...
const techStats = computed(() => {
  ...
  return [
    { label: 'Tiket Selesai', value: completed.toString(), icon: 'check', color: 'text-emerald-600', bg: 'bg-emerald-100' },
    { label: 'Proses Instalasi', value: processing.toString(), icon: 'wrench', color: 'text-blue-600', bg: 'bg-blue-100' },
    { label: 'Antrean Survei', value: pending.toString(), icon: 'tools', color: 'text-amber-600', bg: 'bg-amber-100' },
    { label: 'Total Pelanggan', value: totalCustomers.value.toString(), icon: 'users', color: 'text-indigo-600', bg: 'bg-indigo-100' },
  ]
})
</script>
