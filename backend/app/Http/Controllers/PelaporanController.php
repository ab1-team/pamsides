...
class PelaporanController extends Controller
{
    ...
    public function index()
    {
        ...
        $minTicketYear = $minTicketDate ? Carbon::parse($minTicketDate)->year : null;
    ...
    public function preview(Request $request)
    {
        ...
        if (! empty($request->bulan)) {
            $tglKondisi = Carbon::createFromDate($tahun, $bulan, 1)->endOfMonth()->toDateString();
        } else {
    ...
    public function simpanSaldo(Request $request)
    {
        ...
        if ($bulanInput === null || $bulanInput === '') {
            return $this->recalcSemuaBulan($tahun);
        }

        return $this->recalcSatuBulan($tahun, $bulanInput);
    }

    private function recalcSatuBulan(int $tahun, string $bulan): JsonResponse
    {
        $bulanList = $this->resolveBulanList($bulan);
        if (empty($bulanList)) {
            return response()->json([
                'success' => false,
        ...
        $count = 0;
        DB::transaction(function () use ($tahun, $bulanList, &$count) {
            foreach ($bulanList as $bulanStr) {
                $count += $this->recalcBulan($tahun, $bulanStr);
            }
        });
    ...
    private function recalcSemuaBulan(int $tahun): JsonResponse
    {
        $count = 0;
        DB::transaction(function () use ($tahun, &$count) {
            DB::table('amount')->where('tahun', $tahun)
                ->whereBetween('bulan', ['01', '12'])
                ->delete();
            for ($m = 1; $m <= 12; $m++) {
                $count += $this->recalcBulan($tahun, str_pad((string) $m, 2, '0', STR_PAD_LEFT));
            }
        });

        return response()->json([
            'success' => true,
            'message' => 'Saldo semua bulan berhasil direkalibrasi.',
            'data' => [
                'tahun' => $tahun,
                'jumlah_rekord' => $count,
            ],
    ...
    private function recalcBulan(int $tahun, string $bulan): int
    {
        $endOfMonth = Carbon::create($tahun, (int) $bulan, 1)->endOfMonth()->toDateString();
        $startOfYear = "{$tahun}-01-01";

        $accounts = DB::table('accounts')->get(['id', 'kode_akun']);
        $rows = [];
        ...
        if (! empty($rows)) {
            DB::table('amount')->upsert(
                $rows,
                ['id'],
                ['account_id', 'tahun', 'bulan', 'debit', 'kredit']
            );
        }
    ...
    private function resolveBulanList(string $bulan): array
    {
    ...
    private function cover(array $data)
    {
        ...
        if (! empty($sub) && $sub !== 'DRPY' && $sub !== 'null' && $sub !== 'undefined') {
            $user = User::find($sub);
            if ($user) {
                $namaTeknisi = strtoupper($user->name);
            }
        }
    ...
    private function arus_kas(array $data)
    {
        ...
        $renderNode = function ($node, &$items, &$sub_total) use (&$renderNode, $computeSaldo) {
